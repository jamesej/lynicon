...
(function ($, window, undefined) {
    ...
    $.fn.hopscotchLayout = function (idx) {
        ...
        var layout = { max: -1 };
        var lastRow = $(this).children().length;
        ...
        function construct($container, subLayout, mode) {
            var totSize = 0;
            for (var i = 0; i <= subLayout.max; i++) {
                if (subLayout[i]) {
                    if (subLayout[i]['$item']) {
                        $container.append(subLayout[i].$item);
                        if (mode == 'col') {
                            subLayout[i].$item.css({ float: 'none', width: '100%' });
                        } else {
                            subLayout[i].$item.css({ float: 'left' });
                        }
                    } else {
                        var newMode = mode == 'col' ? 'row' : 'col';
                        var $subCont = $("<div class='hs-container hs-" + newMode + "-container'></div>");
    ...
    $.fn.hopscotchResize = function (stopPoints) {
        var $this = this;
