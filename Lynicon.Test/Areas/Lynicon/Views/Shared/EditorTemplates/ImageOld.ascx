...
<% bool showImg = (!string.IsNullOrEmpty(Model.Url)); %>
<div class="lyn-image">
    <div class="lyn-image-content" <%= showImg ? "" : "style='display:none'" %>>
        <% if (!showImg)
            { %>
        no image
        <% }
    ...
    <div class="lyn-image-url" <%= showImg ? "style='display:none'" : "" %>>
        <span>Url</span>
        <input type="text" class="lyn-file-url" name="<%= ViewData.TemplateInfo.HtmlFieldPrefix %>.Url" value="<%= Model.Url %>" />
    </div>
    <div class='lyn-image-alt' <%= showImg ? "style='display:none'" : "" %>>
