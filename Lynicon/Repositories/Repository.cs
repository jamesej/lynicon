...
namespace Lynicon.Repositories
{
    ...
    public class Repository : TypeRegistry<IRepository>, IRepository
    {
        ...
        public Repository()
        {
            this.DefaultHandler = new ContentRepository(new CoreDataSourceFactory());
            // register ContentRepository for ContentItem
            this.Register(typeof(ContentItem), new ContentRepository(new CoreDataSourceFactory()));
            this.EFProxyCreationEnabled = true;
            this.QueryTimeoutSecs = null;
            this.AvoidConnection = false;
        }

        /// <summary>
        /// Whether the db context creates proxies
        /// </summary>
        public bool EFProxyCreationEnabled { get; set; }
        ...
        public List<bool> Set(List<object> items, Dictionary<string, object> setOptions)
        {
            if (!setOptions.ContainsKey("setAudit") || (bool)setOptions["setAudit"])
            {
                foreach (var aud in items.OfType<IBasicAuditable>())
                {
                    ...
                    aud.UserUpdated = LyniconSecurityManager.Current.UserId;
                }
            }
