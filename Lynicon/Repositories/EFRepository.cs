...
namespace Lynicon.Repositories
{
        ...
        where TContext : DbContext, new()
    {
        ...
        public Dictionary<Type, List<string>> AlwaysIncludes { get; set; }
        ...
        {
            string cacheKey = "_lynicon_request_context_" + typeof(TContext).Name;
            if (this.ContextLifetimeMode == ContextLifetimeMode.PerRequest
                && HttpContext.Current.Items != null
                && HttpContext.Current.Items.Contains(cacheKey))
                return (TContext)HttpContext.Current.Items[cacheKey];

            var db = GetDbInner();

            if (this.ContextLifetimeMode == ContextLifetimeMode.PerRequest
                && HttpContext.Current.Items != null)
                HttpContext.Current.Items.Add(cacheKey, db);
        ...
        public EFRepository()
        {
            ...
            AlwaysIncludes = new Dictionary<Type, List<string>>();
        }
            ...
            where TQuery : class
        {
            ...
            try
            {
                ...
                qed.Source = ApplyIncludes(typeof(T), (DbSet)((TContext)db).Set<T>());
        ...
        public DbQuery ApplyIncludes(Type t, DbQuery set)
        {
            Type containerType = t.ContentType();
            if (AlwaysIncludes.ContainsKey(containerType))
            {
                foreach (string incl in AlwaysIncludes[containerType])
                    set = set.Include(incl);
            }
        ...
        public virtual int GetCount<T>(IEnumerable<Type> types, Func<IQueryable<T>, IQueryable<T>> queryBody) where T : class
        {
            ...
            if (types.Count() == 1)
            {
                ...
                try
                {
                    var qed = new QueryEventData<IQueryable>
                    {
                        Source = db.Set(typeof(T).ContentType()),
