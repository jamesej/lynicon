...
namespace Lynicon.Repositories
{
    ...
    public class ContentRepository : IRepository
    {
        ...
        /// <summary>
        /// The factory this repository uses to generate its data source
        /// </summary>
        ...
        {
            return ContainerType(t).GetProperty("Identity");
        }
        ...
        protected Type ContainerType()
        {
            ...
            {
                Type ci = typeof(ContentItem);
                if (CompositeTypeManager.Instance.ExtendedTypes.ContainsKey(ci))
                    ci = CompositeTypeManager.Instance.ExtendedTypes[ci];
                containerType = ci;
            }
            return containerType;
        }
        public Type ContainerType(Type t)
        {
            return ContainerType();
        }
        ...
        public IEnumerable<T> Get<T>(Type targetType, IEnumerable<ItemId> ids) where T : class
        {
            ...
            using (var dataSource = DataSourceFactory.Create(isSummary))
            {
                ...
                if (!Lynicon.Extensibility.Cache.IsTotalCached(typeof(T), isSummary))
