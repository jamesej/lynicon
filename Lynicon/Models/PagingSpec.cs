...
namespace Lynicon.Models
{
    ...
    public class PagingSpec
    {
        ...
        /// </summary>
        /// <param name="currentUri">Current url</param>
        /// <param name="page">Page number</param>
        /// <returns>Resulting url</returns>
        public string GetUrl(Uri currentUri, int page)
        {
            int skip = page * Take;
            if (skip < 0) skip = 0;
            if (skip >= Total) skip = (int)Math.Floor((double)(Total / Take)) * Take;

            var url = UrlX.EnsureQueryKeyValue(currentUri.OriginalString, "$skip", skip.ToString());
